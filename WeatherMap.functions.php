...
function wm_interpolate($v1, $v2, $percent)
{
 ...
 * @return float[]
 */
function wm_calc_offset($offsetstring, $width, $height)
{
...
class WMGraphics { 
    ...
    function draw_marker_cross($im, $col, $x, $y, $size = 5)
    {
    ...
    function draw_marker_box($im, $col, $x, $y, $size = 10)
    {
        $points = array ();
    ...
    function draw_marker_circle($im, $col, $x, $y, $size = 10)
    {
...
class WMCurve {
    ...
    // Given a list of key points, calculate a curve through them
    ...
    // add in the very first point manually (the calc function skips this one to avoid duplicates, which mess up the distance stuff)
        ...
        for ($i = 0; $i < ($npoints - 3); $i++) {
            list($newpoints, $distance, $np) =
                WMCurve::calculate_catmull_rom_span($np, $distance, $pointsperspan, $xarray[$i],
