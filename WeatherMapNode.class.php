...
class WeatherMapNode extends WeatherMapItem
{
    ...
    function pre_render($im, &$map)
    {
        ...
        if ($this->iconfile != '') {
            ...
            if ($this->iconfile == 'rbox' || $this->iconfile == 'box'
                ...
                if ($this->iconfile == 'box') {
                    if ($fill !== null && !$fill->is_none()) {
                        imagefilledrectangle($icon_im, 0, 0, $this->iconscalew - 1,
                            $this->iconscaleh - 1, $fill->gdallocate($icon_im));
                    }

                    if ($ink !== null && !$ink->is_none()) {
                        imagerectangle($icon_im, 0, 0, $this->iconscalew - 1,
                            $this->iconscaleh - 1, $ink->gdallocate($icon_im));
                    }
                }
                ...
                if ($this->iconfile == 'round') {
                    ...
                    if ($fill !== null && !$fill->is_none()) {
                        imagefilledellipse($icon_im, $rx, $ry, $rx * 2, $ry * 2,
                            $fill->gdallocate($icon_im));
                    }

                    if ($ink !== null && !$ink->is_none()) {
                        imageellipse($icon_im, $rx, $ry, $rx * 2, $ry * 2,
                            $ink->gdallocate($icon_im));
                    }
                }
                ...
                if ($this->iconfile == 'nink') {
                    ...
                    imagefilledarc($icon_im, $rx - 1, $ry, $size, $size, 270, 90,
                        $col1->gdallocate($icon_im), IMG_ARC_PIE);
                    imagefilledarc($icon_im, $rx + 1, $ry, $size, $size, 90, 270,
                        $col2->gdallocate($icon_im), IMG_ARC_PIE);

                    imagefilledarc($icon_im, $rx - 1, $ry + $quarter, $quarter * 2,
                        $quarter * 2, 0, 360, $col1->gdallocate($icon_im), IMG_ARC_PIE);
                    imagefilledarc($icon_im, $rx + 1, $ry - $quarter, $quarter * 2,
                        $quarter * 2, 0, 360, $col2->gdallocate($icon_im), IMG_ARC_PIE);
                    ...
                    if ($ink !== null && !$ink->is_none()) {
                        ...
                        $instr =
                            $map->ProcessString("{node:this:bandwidth_in:%.1k}", $this);
                        $outstr =
                            $map->ProcessString("{node:this:bandwidth_out:%.1k}", $this);

                        list($twid, $thgt) = $map->myimagestringsize($font, $instr);
                        $map->myimagestring($icon_im, $font, $rx - $twid / 2,
                            $ry - $quarter + ($thgt / 2), $instr,
                            $ink->gdallocate($icon_im));

                        list($twid, $thgt) = $map->myimagestringsize($font, $outstr);
                        $map->myimagestring($icon_im, $font, $rx - $twid / 2,
                            $ry + $quarter + ($thgt / 2), $outstr,
                            $ink->gdallocate($icon_im));
                ...
                if ($this->iconfile == 'inpie' || $this->iconfile == 'outpie') {
                    ...
                    if ($ink !== null && !$ink->is_none()) {
                        imagefilledarc($icon_im, $rx, $ry, $rx * 2, $ry * 2, 0,
                            $segment_angle, $ink->gdallocate($icon_im), IMG_ARC_PIE);
                    }
                ...
                if (is_readable($this->iconfile)) {
                    ...
                    if ($icon_im) {
                        ...
                        if (($this->iconscalew * $this->iconscaleh) > 0) {
                            ...
                            imagecopyresampled($scaled, $icon_im, 0, 0, 0, 0, $new_width,
                                $new_height, $icon_w, $icon_h);
            ...
            if ($icon_im) {
                ...
                $map->nodes[$this->name]->boundingboxes[] = array (
                    $icon_x1,
                    $icon_y1,
                    $icon_x2,
                    $icon_y2
        ...
        if (isset($icon_im)) {
            imagecopy($node_im, $icon_im, $icon_x1, $icon_y1, 0, 0, imagesx($icon_im),
                imagesy($icon_im));
            imagedestroy($icon_im);
        }
        ...
        if ($this->label != '') {
            ...
            if (!$col->is_none()) {
                imagefilledrectangle($node_im, $label_x1, $label_y1, $label_x2, $label_y2,
            ...
            if ($this->selected) {
                ...
                wimagerectangle($node_im, $label_x1 + 1, $label_y1 + 1, $label_x2 - 1,
            ...
            $map->myimagestring($node_im, $this->labelfont, $txt_x, $txt_y,
                $this->proclabel, $txcol->gdallocate($node_im), $this->labelangle);
        }
    ...
    function NewDraw($im, &$map)
    {
        ...
        if (isset($this->image)) {
            imagealphablending($im, true);
            imagecopy($im, $this->image, $this->x - $this->centre_x,
                $this->y - $this->centre_y, 0, 0, imagesx($this->image),
                imagesy($this->image));
        }
    }
        ...
        if ($this->config_override != '') {
            ...
            if ($val != $comparison) {
                if ($this->relative_to == '') {
                    ...
                    if ($this->polar) {
                        $output .= "\tPOSITION " . $this->relative_to . " "
                            . $this->original_x . "r" . $this->original_y . "\n";
                    } else {
                        $output .= "\tPOSITION " . $this->relative_to . " "
                            . $this->original_x . " " . $this->original_y . "\n";
                    }
                }
            }
    ...
    function asJS()
    {
        ...
        if (preg_match("/^(none|nink|inpie|outpie|box|rbox|gauge|round)$/",
            $this->iconfile)) {
