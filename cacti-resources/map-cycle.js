...
WMcycler = {
	...
	start: function (initialData) {        	
		...
		$("#cycle_pause").click( function() { that.pauseAction(); } );
		$("#cycle_next").click( function() { that.nextAction(); } );
		$("#cycle_prev").click( function() { that.previousAction(); } );
		...
		if (this.nmaps > 0) {
			...
			this.timer_counter = setInterval( function() { that.counterHandler(); }, 200);

			// when to reload the whole page (with new map data)
			this.timer_reloader = setTimeout( function() { that.reload(); }, this.poller_cycle);
		}
		...
		if (this.fullscreen) {
			...
			// $(document).mousemove( function () {$("#wmcyclecontrolbox").fadeIn(100); }  );
