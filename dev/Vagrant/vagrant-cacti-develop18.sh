...
apt-get update -y
## For 'real' install:
apt-get install -y mariadb-server snmp rrdtool apache2 php-gettext php-xdebug unzip
...
apt-get install -y git subversion make xsltproc imagemagick zip curl phpunit nodejs npm pandoc rsync php-ast
for PHP_VER in ${PHP_VERSIONS}; do
...
EOF
    ...
    bash -c "cat > /etc/mysql/mariadb.conf.d/99-cacti.cnf" <<'EOF'
[mysqld]
