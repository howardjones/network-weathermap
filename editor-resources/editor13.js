...
var wmEditor = {
    ...
    showLinkProperties: function(name) {
        ...
        $("#mainarea").after(template(Links[name]));
    ...
    hideAllDialogs: function() {
        ...
        jQuery(".dlgProperties").modal('hide');
    },
...
function click_handler(e) {
    ...
    // we've got a command queued, so do the appropriate thing
        ...
        if (objecttype == 'NODE' && jQuery("#action").val() == 'add_link') {
            // document.getElementById('param').value = NodeIDs[objectid];
            jQuery("#param").val(NodeIDs[objectid]);
            document.frmMain.submit();
        }

        else if (objecttype == 'NODE' && jQuery("#action").val() == 'add_link2') {
            // document.getElementById('param').value = NodeIDs[objectid];
            jQuery("#param").val(NodeIDs[objectid]);
            document.frmMain.submit();
        }
...
function cactipicker() {
    ...
    jQuery("#dlgDataPicker").modal("show");
    return;
...
function manage_colours() {
    ...
    jQuery("#action").val("set_map_colours");
...
function prefs() {
    ...
    jQuery("#action").val("editor_settings");
...
function move_node() {
    ...
    jQuery("#tb_help").text('Click on the map where you would like to move the node to.');
...
function map_properties() {
    ...
    jQuery("#action").val("set_map_properties");
...
function map_style() {
    ...
    jQuery("#action").val("set_map_style");
...
function real_position_legend(scalename) {
    document.getElementById('tb_help').innerText = 'Click on the map where you would like to put the legend.';
    // document.getElementById('action').value = "place_legend";
    jQuery("#action").val("place_legend");
    jQuery("#param").val(scalename);
...
function align_link_h() {
    jQuery("#action").val("link_align_horizontal");
    document.frmMain.submit();
}

function align_link_v() {
    jQuery("#action").val("link_align_vertical");
    document.frmMain.submit();
}
