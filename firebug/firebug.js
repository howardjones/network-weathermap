...
// ********************************************************************************************
        ...
        function toggleConsole(forceOpen)
    {
        ...
        if (consoleFrame) consoleFrame.style.visibility = frameVisible
            ? "visible" : "hidden";
        else waitForBody();
    }
        ...
        function layout()
    {
        ...
        var height =
            consoleFrame.offsetHeight - (toolbar.offsetHeight + commandLine.offsetHeight);
        ...
        function flush()
    {
        ...
        for (var i = 0;
            i < queue.length; ++i) writeMessage(queue[i][0], queue[i][1], queue[i][2]);
    }
        ...
        function writeMessage(message, className, handler)
    {
        ...
        if (isScrolledToBottom) consoleBody.scrollTop = consoleBody.scrollHeight
    ...
    {
        var container =
            groupStack.length ? groupStack[groupStack.length - 1] : consoleBody;
...
// ********************************************************************************************
        ...
        function appendNull(object, html) { html.push('<span class="objectBox-null">',
            escapeHTML(objectToString(object)), '</span>'); }
        ...
        function appendFloat(object, html) { html.push('<span class="objectBox-number">',
            escapeHTML(objectToString(object)), '</span>'); }
        ...
        function appendSelector(object, html)
    {
        ...
        html.push('<span class="selectorTag">', escapeHTML(object.nodeName.toLowerCase()),
            '</span>');

        if (object.id) html.push('<span class="selectorId">#', escapeHTML(object.id),
            '</span>');

        if (object.className) html.push('<span class="selectorClass">.',
            escapeHTML(object.className), '</span>');
        ...
        if (node.nodeType == 1) {
            ...
            if (node.firstChild) {
                ...
                for (var child = node.firstChild;
                    child; child = child.nextSibling) appendNode(child, html);
...
// ********************************************************************************************
        ...
        function cancelEvent(event)
    {
        ...
        function onError(msg, href, lineNo)
        {
            ...
            html.push('<span class="errorMessage">', msg, '</span>',
                '<div class="objectBox-sourceLink">', fileName, ' (line ', lineNo,
                ')</div>');
        ...
        function onKeyDown(event)
    {
        if (event.keyCode == 123) toggleConsole();
            ...
            && (event.metaKey || event.ctrlKey)) focusCommandLine();
        else return;
    ...
    {
        var win = document.all ? event.srcElement.ownerDocument.parentWindow
            : event.target.ownerDocument.defaultView;

        var clientY = event.clientY;

        if (win
            != win.parent) clientY += win.frameElement ? win.frameElement.offsetTop : 0;
        ...
        function onCommandLineKeyDown(event)
    {
        ...
        if (document.documentElement.getAttribute("debug") == "true") toggleConsole(true);
    })();
}