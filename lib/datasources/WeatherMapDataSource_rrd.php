...
class WeatherMapDataSource_rrd extends WeatherMapDataSource
{
    function Init(&$map)
    {
        ...
        if ($map->context === 'cacti') {
            wm_debug('RRD DS: path_rra is ' . $config['rra_path']
            ...
            foreach (array (
                ...
                if ($dsnames[$dir] !== '-') {
                    ...
                    $SQL = "select * from weathermap_data where rrdfile='"
                        ...
                        . mysql_real_escape_string($dsnames[$dir]) . "'";
                    ...
                    $SQLcheck =
                        ...
                        . mysql_real_escape_string($dsnames[$dir]) . "'";
                    $SQLvalid =
                        "select data_template_rrd.data_source_name from data_template_data,data_template_rrd where data_template_data.local_data_id=data_template_rrd.local_data_id and data_template_data.data_source_path='"
                        . mysql_real_escape_string($db_rrdname) . "'";
...
// OK, the straightforward query for data failed, let's work out why, and add the new data source if necessary
                    if (!isset($result['id'])) {
                        wm_debug(
                            "RRD ReadData: poller_output - Adding new weathermap_data row for $db_rrdname:"
                            . $dsnames[$dir] . "\n");
                        ...
                        if (!isset($result['local_data_id'])) {
                            ...
                            if (count($fields) > 0) {
                                wm_warn("RRD ReadData: poller_output: " . $dsnames[$dir]
                                    . " is not a valid DS name for $db_rrdname - valid names are: "
                                    . join(", ", $fields) . " [WMRRD07]\n");
                            } else {
                                wm_warn(
                                    "RRD ReadData: poller_output: $db_rrdname is not a valid RRD filename within this Cacti install. <path_rra> is $path_rra [WMRRD08]\n");
                            }
                        } else {
...
// (and to allow the cacti: DS plugin to use the same table, too)
                            ...
                            wm_debug(
                                "RRD ReadData: poller_output - Adding new weathermap_data row for data source ID "
                                . $result['local_data_id'] . "\n");
                            db_execute($SQLins);
                        }
                    } else { // the data table line already exists
                        wm_debug(
                            "RRD ReadData: poller_output - found weathermap_data row\n");

                        // if the result is valid, then use it
                        if (($result['sequence'] > 2)
                            && ($result['last_time'] > $worst_time)) {
                            ...
                            $data[$dir] = 0;
                            wm_debug(
                                "RRD ReadData: poller_output - data is either too old, or too new\n");
                        }
...
// first, see if the weathermap_data entry *has* a local_data_id. If not, we need to update this entry.
                        ...
                        if (!isset($result['local_data_id'])
                            || $result['local_data_id'] == 0) {
                            $r2 = db_fetch_row($SQLcheck);

                            if (isset($r2['local_data_id'])) {
                                $ldi = $r2['local_data_id'];
                                wm_debug(
                                    "RRD ReadData: updated  local_data_id for wmdata.id="
                                    . $result['id'] . "to $ldi\n");
// put that in now, so that we can skip this step next time
                                db_execute("update weathermap_data set local_data_id="
                                    . $r2['local_data_id'] . " where id="
                                    . $result['id']);
                            }
                        } else {
        ...
        wm_debug( sprintf("RRD ReadData: poller_output: Returning (%s, %s, %s)\n",
        ...
        &$data_time, &$item)
    {
        ...
        if ((1 == 0)
            && extension_loaded('RRDTool')) // fetch the values via the RRDtool Extension
        {
...
# rrdtool graph /dev/null -f "" -s now-30d -e now DEF:in=../rra/atm-sl_traffic_in_5498.rrd:traffic_in:AVERAGE DEF:out=../rra/atm-sl_traffic_in_5498.rrd:traffic_out:AVERAGE VDEF:avg_in=in,AVERAGE VDEF:avg_out=out,AVERAGE PRINT:avg_in:%lf PRINT:avg_out:%lf
    ...
    function wmrrd_read_from_real_rrdtool_aggregate($rrdfile, $cf, $aggregatefn, $start,
        $end, $dsnames, &$data, &$map, &$data_time, &$item)
    {
        ...
        if (isset($pipe)) {
            ...
            if ($linecount > 1) {
                foreach ($lines as $line) {
                    if (preg_match('/^\'(IN|OUT)\s(\-?\d+[\.,]?\d*e?[+-]?\d*:?)\'$/i',
                        $line, $matches)) {
            ...
            wm_warn("RRD ReadData: failed to open pipe to RRDTool: " . $php_errormsg
                . " [WMRRD04]\n");
        }
        
           wm_debug( sprintf("RRD ReadDataFromRealRRDAggregate: Returning (%s, %s, %s)\n",
		        string_or_null($data[IN]),
		        string_or_null($data[OUT]),
		        $data_time
        	));
...
//  - this allows for early failure, and also tolerance of empty data in other parts of an rrd (like smokeping uptime)
        ...
        if (isset($pipe)) {
            ...
            if ((in_array($dsnames[IN], $heads) || $dsnames[IN] == '-')
                ...
                wm_warn("RRD ReadData: At least one of your DS names (" . $dsnames[IN]
                    . " and " . $dsnames[OUT]
                    . ") were not found, even though there was a valid data line. Maybe they are wrong? Valid DS names in this file are: $names [WMRRD06]\n");
            }
        } else {
            wm_warn("RRD ReadData: failed to open pipe to RRDTool: " . $php_errormsg
                . " [WMRRD04]\n");
        ...
        wm_debug( sprintf("RRD ReadDataFromRealRRD: Returning (%s, %s, %s)\n",
		        string_or_null($data[IN]),
		        string_or_null($data[OUT]),
		        $data_time
        	));
...
//   because there won't be valid data in the weathermap_data table yet.
        if (($dsnames[IN] != '-' && $data[IN] === null)
            ...
            if (file_exists($rrdfile)) {
                wm_debug("RRD ReadData: Target DS names are " . $dsnames[IN] . " and "
                    . $dsnames[OUT] . "\n");

                $values = array ();

                if ((1 == 0) && extension_loaded(
                    'RRDTool')) // fetch the values via the RRDtool Extension
                {
                    WeatherMapDataSource_rrd::wmrrd_read_from_php_rrd($rrdfile, $cfname,
                        $start, $end, $dsnames, $data, $map, $data_time, $item);
                } else {
                    if ($aggregatefunction != '') {
                        WeatherMapDataSource_rrd::wmrrd_read_from_real_rrdtool_aggregate(
                            $rrdfile, $cfname, $aggregatefunction, $start, $end, $dsnames,
                                $data, $map, $data_time, $item);
                    } else {
                        // do this the tried and trusted old-fashioned way
                        WeatherMapDataSource_rrd::wmrrd_read_from_real_rrdtool($rrdfile,
                            $cfname, $start, $end, $dsnames, $data, $map, $data_time,
                            $item);
                    }
                }
            } else {
        ...
        if ($data[OUT] !== null) {
  		...
  		wm_debug( sprintf("RRDTool ReadData: Returning (%s, %s, %s)\n",
		        string_or_null($data[IN]),
		        string_or_null($data[OUT]),
		        $data_time
        	));
