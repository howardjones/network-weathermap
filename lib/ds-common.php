...
function UpdateCactiData(&$item, $local_data_id)
{
    ...
    $r3 =
        db_fetch_assoc(
            sprintf(
#                "select data_local.host_id, field_name,field_value from data_local,host_snmp_cache  where data_local.id=%d and data_local.host_id=host_snmp_cache.host_id and data_local.snmp_index=host_snmp_cache.snmp_index and data_local.snmp_query_id=host_snmp_cache.snmp_query_id",
	...
	$to_set['cacti_host_id'] = intval($vv['host_id']);
        # $item->add_note("cacti_host_id", intval($vv['host_id']));
	}
	...
	$to_set["cacti_graph_id"] = intval($r4['local_graph_id']);
        # $item->add_note("cacti_graph_id", intval($r4['local_graph_id']));
	}
