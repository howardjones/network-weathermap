        ...
        if ($set_speed != 0) {
            ...
            if ($speed > 0) {
                ...
                if ($item->my_type() == 'NODE') {
                    $map->nodes[$item->name]->maxValues[IN] = $speed;
                    $map->nodes[$item->name]->maxValues[OUT] = $speed;
                }

                if ($item->my_type() == 'LINK') {
                    $map->links[$item->name]->maxValues[IN] = $speed;
                    $map->links[$item->name]->maxValues[OUT] = $speed;
                }
            }
        }
