 ...
 */
     ...
     * @cat Plugins/blockUI
     */
    $.unblockUI = function() { $.blockUI.impl.remove(window); };
     ...
     * @cat Plugins/blockUI
     */
    $.fn.block = function(msg, css) { return this.each(function()
    {
        if (!this.$pos_checked) {
     ...
     */
    $.fn.unblock = function()
        { return this.each(function() { $.blockUI.impl.remove(this); }); };
    ...
    $.blockUI.impl = {
        ...
        install: function(el, msg, css, displayMode, noalpha)
        {
            ...
            msg = msg ? (msg.nodeType ? $(msg) : msg) : full
                ? $.blockUI.defaults.pageMessage : $.blockUI.defaults.elementMessage;

            if (displayMode) var basecss =
                jQuery.extend({ }, $.blockUI.defaults.displayBoxCSS);
            else var basecss = jQuery.extend({ }, full ? $.blockUI.defaults.pageMessageCSS
                : $.blockUI.defaults.elementMessageCSS);
            css = jQuery.extend(basecss, css || { });
            var f = ($.browser.msie)
                ? $(
                    '<iframe class="blockUI" style="z-index:1000;border:none;margin:0;padding:0;position:absolute;width:100%;height:100%;top:0;left:0" src="javascript:false;document.write(\'\');"></iframe>')
                : $('<div class="blockUI" style="display:none"></div>');
            var w =
                $(
                    '<div class="blockUI" style="z-index:1001;cursor:wait;border:none;margin:0;padding:0;width:100%;height:100%;top:0;left:0"></div>');
            var m =
                full
                    ? $(
                        '<div class="blockUI blockMsg" style="z-index:1002;cursor:wait;padding:0;position:fixed"></div>')
                    : $(
...
// ie7 must use absolute positioning in quirks mode and to account for activex issues (when scrolling)
            ...
            if (this.ie6 || expr) {
                ...
                if (full && $.blockUI.defaults.ie6Stretch
                    && $.boxModel) $('html,body').css('height', '100%');

                // fix ie6 problem when blocked element has a border width
                if ((this.ie6 || !$.boxModel) && !full) {
                    var t = this.sz(el, 'borderTopWidth'),
                        l = this.sz(el, 'borderLeftWidth');
                    var fixT = t ? '(0 - ' + t + ')' : 0;
                    var fixL = l ? '(0 - ' + l + ')' : 0;
                }
            ...
            if (displayMode) {
                ...
                $([
                    ...
                    w[0],
                    m[0]
                ]).removeClass('blockUI').addClass('displayBox');
        ...
        boxRemove: function(el)
        {
            ...
            if (this.boxCallback) this.boxCallback(this.box);
        ...
        boxHandler: function(e)
        {
            if ((e.keyCode && e.keyCode == 27)
                || (e.type == 'click' && $(e.target).parents('div.blockMsg').length
                    == 0)) $.blockUI.impl.boxRemove();
            return true;
        },
        ...
        bind: function(b, el)
        {
            ...
            $.each([
                ...
                'keypress',
                'click'
            ], function(i, o) { $e[b ? 'bind' : 'unbind'](o, $.blockUI.impl.handler); });
        },
        focus: function(back)
        {
            if (!$.blockUI.impl.pageBlockEls) return;
            var e = $.blockUI.impl.pageBlockEls[back === true
                ? $.blockUI.impl.pageBlockEls.length - 1 : 0];
