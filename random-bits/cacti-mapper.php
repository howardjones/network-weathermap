...
foreach ($interfaces as $key => $int) {
    if (!isset($int['ip'])) {
        ...
        $interfaces[$key]['nicename'] = (isset($int['name'])
            ? $int['name'] : (isset($int['descr']) ? $int['descr'] : (isset($int['alias'])
                ? $int['alias'] : "Interface #" . $int['index'])));
    }
}
...
foreach ($interfaces as $key => $int) {
    if (!isset($int['netmask'])) {
        ...
        if ($count < 100) {
            print "Fetching Netmask via SNMP for Host " . $int['host'] . "//" . $int['ip']
                . " from $oid\n";
            $result = cacti_snmp_get($hosts[$hostid]["hostname"],
                $hosts[$hostid]["snmp_community"], $oid,
                $hosts[$hostid]["snmp_version"], $hosts[$hostid]["snmp_username"],
...
foreach ($interfaces as $key => $int) {
    if (isset($int['netmask'])) {
        $network =
            get_network($int['ip'], $int['netmask']) . "/" . get_cidr($int['netmask']);
...
foreach ($networks as $network => $members) {
    ...
    if (sizeof($members) == 2) {
        ...
        $link_config .= "\tNODES node_" . $interfaces[$members[0]]['host'] . " node_"
            . $interfaces[$members[1]]['host'] . "\n";
