...
function disabled_plugin_weathermap_install()
{
    ...
    api_plugin_register_hook('weathermap', 'config_arrays', 'weathermap_config_arrays',
        'setup.php');
    api_plugin_register_hook('weathermap', 'draw_navigation_text',
        'weathermap_draw_navigation_text', 'setup.php');
    api_plugin_register_hook('weathermap', 'config_settings',
        'weathermap_config_settings', 'setup.php');
    api_plugin_register_hook('weathermap', 'top_header_tabs', 'weathermap_show_tab',
        'setup.php');
    api_plugin_register_hook('weathermap', 'top_graph_header_tabs', 'weathermap_show_tab',
        'setup.php');

    api_plugin_register_hook('weathermap', 'poller_bottom', 'weathermap_poller_bottom',
        'setup.php');
    api_plugin_register_hook('weathermap', 'poller_top', 'weathermap_poller_top',
        'setup.php');
    api_plugin_register_hook('weathermap', 'poller_output', 'weathermap_poller_output',
        'setup.php');

    api_plugin_register_hook('weathermap', 'top_graph_refresh',
        'weathermap_top_graph_refresh', 'setup.php');
    api_plugin_register_hook('weathermap', 'page_title', 'weathermap_page_title',
        'setup.php');
    api_plugin_register_hook('weathermap', 'page_head', 'weathermap_page_head',
        'setup.php');
...
function plugin_init_weathermap()
{
    ...
    $plugin_hooks['draw_navigation_text']['weathermap'] =
        'weathermap_draw_navigation_text';
...
//	}
    if (preg_match('/plugins\/weathermap\//', $_SERVER['REQUEST_URI'], $matches)) {
        print
            '<LINK rel="stylesheet" type="text/css" media="screen" href="weathermap-cacti-plugin.css">';
    }
}
...
function weathermap_page_title($t)
{
    if (preg_match('/plugins\/weathermap\//', $_SERVER['REQUEST_URI'], $matches)) {
        ...
        if (preg_match(
            '/plugins\/weathermap\/weathermap-cacti-plugin.php\?action=viewmap&id=([^&]+)/',
...
function weathermap_config_settings()
{
    ...
    $temp = array (
        ...
        "weathermap_thumbsize" => array (
            ...
            "description" =>
        "The maximum width or height for thumbnails in thumbnail view, in pixels. Takes effect after the next poller run.",
            "method" => "textbox",
            "max_length" => 5,
        ),
        "weathermap_cycle_refresh" => array (
            ...
            "description" =>
        "How often to refresh the page in Cycle mode. Automatic makes all available maps fit into 5 minutes.",
            "method" => "drop_array",
            "array" => array (
                0 => "Automatic",
                5 => "5 Seconds",
        ...
        "weathermap_output_format" => array (
            ...
            "description" =>
        "What format do you prefer for the generated map images and thumbnails?",
            "method" => "drop_array",
            "array" => array (
                'png' => "PNG (default)",
                'jpg' => "JPEG",
        ...
        "weathermap_render_period" => array (
            ...
            "description" =>
        "How often do you want Weathermap to recalculate it's maps? You should not touch this unless you know what you are doing! It is mainly needed for people with non-standard polling setups.",
            "method" => "drop_array",
            "array" => array (
        ...
        "weathermap_quiet_logging" => array (
            ...
            "description" =>
        "By default, even in LOW level logging, Weathermap logs normal activity. This makes it REALLY log only errors in LOW mode.",
            "method" => "drop_array",
            "array" => array (
                0 => "Chatty (default)",
                1 => "Quiet"
...
// - saves a handful of db hits per request!
    if (($dbversion == "") || (preg_match("/dev$/", $myversion))
        || ($dbversion != $myversion)) {
        ...
        $sql[] =
            "update weathermap_maps set filehash=LEFT(MD5(concat(id,configfile,rand())),20) where filehash = '';";
        ...
        $sql[] =
            "update weathermap_maps set sortorder=id where sortorder is null or sortorder=0;";
...
function weathermap_tree_item_render($leaf)
{
    ...
    $map =
        ...
        . $_SESSION["sess_user_id"] . " or userid=0) and weathermap_maps.id="
        . $leaf['item_id']);
...
{
    $description = db_fetch_cell("select titlecache from weathermap_maps where id="
        . intval($item_id));

    if ($description == '') {
        $configfile = db_fetch_cell("select configfile from weathermap_maps where id="
            . intval($item_id));
...
function weathermap_tree_item_edit($tree_item)
{
    ...
    print
        "<td width='50%'><font class='textEditTitle'>Map</font><br />Choose which weathermap to add to the tree.</td><td>";
    form_dropdown("item_id",
        db_fetch_assoc(
            "select id,CONCAT_WS('',titlecache,' (',configfile,')') as name from weathermap_maps where active='on' order by titlecache, configfile"),
        "name", "id", $tree_item['item_id'], "", "0");
    print "</td></tr>";
    form_alternate_row_color($colors["form_alternate1"], $colors["form_alternate2"], 1);
    print
        "<td width='50%'><font class='textEditTitle'>Style</font><br />How should the map be displayed?</td><td>";
    print
        "<select name='item_options'><option value=1>Thumbnail</option><option value=2>Full Size</option></select>";
    print "</td></tr>";
}
...
function weathermap_show_tab()
{
    ...
    if ((db_fetch_assoc(
        ...
        print '<a href="' . $config['url_path']
            . 'plugins/weathermap/weathermap-cacti-plugin.php"><img src="'
            . $config['url_path'] . 'plugins/weathermap/images/' . $prefix
            . 'tab_weathermap';

        // if we're ON a weathermap page, print '_red'
        if (preg_match('/plugins\/weathermap\/weathermap-cacti-plugin.php/',
            $_SERVER['REQUEST_URI'], $matches)) {
            print "_red";
        }
...
// $nav["weathermap-cacti-plugin-mgmt.php:perms_edit"] = array("title" => "Weathermap Management", "mapping" => "index.php:", "url" => "weathermap-cacti-plugin-mgmt.php", "level" => "1");
    ...
    $nav["weathermap-cacti-plugin-editor.php:"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    );

    $nav["weathermap-cacti-plugin-editor.php:newmap"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:newmapcopy"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:font_samples"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:draw"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:show_config"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:fetch_config"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:set_link_config"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:set_node_config"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:set_node_properties"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:set_link_properties"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:set_map_properties"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:set_map_style"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:add_node"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:add_link"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:add_link2"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:delete_link"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:delete_node"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:clone_node"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:place_legend"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:place_stamp"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:via_link"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:move_node"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:link_align_horizontal"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:link_align_vertical"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
    ...
    $nav["weathermap-cacti-plugin-editor.php:editor_settings"] = array (
        "title" => "Weathermap Management",
        "mapping" => "index.php:",
        "url" => "weathermap-cacti-plugin-editor.php",
        "level" => "1"
...
// new version works with *either* a local_data_id or rrdfile in the weathermap_data table, and returns BOTH
    $requiredlist =
        db_fetch_assoc(
            "select distinct weathermap_data.id, weathermap_data.last_value, weathermap_data.last_time, weathermap_data.data_source_name, data_template_data.data_source_path, data_template_data.local_data_id, data_template_rrd.data_source_type_id from weathermap_data, data_template_data, data_template_rrd where weathermap_data.local_data_id=data_template_data.local_data_id and data_template_rrd.local_data_id=data_template_data.local_data_id and weathermap_data.local_data_id<>0;");
...
# let's make an index from local_data_id to filename, and then use local_data_id as the key...
    ...
    foreach (array_keys($rrd_update_array) as $key) {
        if (isset($rrd_update_array[$key]['times'])
            && is_array($rrd_update_array[$key]['times'])) {
# if($logging >= POLLER_VERBOSITY_DEBUG) cacti_log("WM poller_output: Adding $key",true,"WEATHERMAP");
    ...
    foreach ($requiredlist as $required) {
        ...
        if ($logging >= POLLER_VERBOSITY_DEBUG)
            cacti_log("WM poller_output: Looking for $file ($local_data_id) ("
                . $required['data_source_path'] . ")\n", true, "WEATHERMAP");

        if (isset($rrd_update_array[$file]) && is_array($rrd_update_array[$file])
            && isset($rrd_update_array[$file]['times'])
            && is_array($rrd_update_array[$file]['times'])
            && isset($rrd_update_array{$file
                }['times'][key($rrd_update_array[$file]['times'])]{$dsname})) {
            $value = $rrd_update_array{$file
                }['times'][key($rrd_update_array[$file]['times'])]{$dsname};
...
// reboots very well, but it should improve it for drops.
            ...
            db_execute(
                "UPDATE weathermap_data SET last_time=$newtime, last_calc='$newvalue', last_value='$newlastvalue',sequence=sequence+1  where id = "
                . $required['id']);
