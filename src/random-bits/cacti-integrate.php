...
//
    ...
    if ($mappingsfile != "") {
        ...
        $fileHandle = fopen($mappingsfile, "ra");
        while (!feof($fileHandle)) {
            $line = fgets($fileHandle, 2000);
            $parts = explode("\t", $line);
...
//  we want links where at least one of the nodes has a cacti_id, and where either interface_in or interface_out is set
    foreach ($map->links as $link) {
        if (isset($link->a)) {
            ...
            $nodeACactiID = intval($map->nodes[$nodeA]->get_hint("cacti_id"));
            $nodeBCactiID = intval($map->nodes[$nodeB]->get_hint("cacti_id"));

            print "LINK $name\n";

            if (count($link->targets) == 0 || $overwrite_targets) {
                if ((($nodeACactiID + $nodeBCactiID) > 0) && ($int_out . $int_in == '')) {
                ...
                if ($nodeACactiID > 0 && $int_out != '') {
                    print "  We'll use the A end.\n";
                    $tgt_interface = $int_out;
                    $tgt_host = $nodeACactiID;
                    # $ds_names = ":traffic_in:traffic_out";
                    $ds_names = "";
                } elseif ($nodeBCactiID > 0 && $int_in != '') {
                    print "  We'll use the B end and reverse it.\n";
                    $tgt_interface = $int_in;
                    $tgt_host = $nodeBCactiID;
                    $ds_names = ":traffic_out:traffic_in";
                } else {
                    print "  No useful ends on this link - fill in more detail (host id, IP) on either NODE $nodeA or $nodeB\n";
                }
                    ...
                    foreach ($int_list as $interface) {
                        ...
                        if ($tgt != "") {
                            ...
                            if ($use_fc_hack) {
                                ...
                                // DS name
                                if ($map->nodes[$nodeA]->get_hint("_is_brocade") == 1) {
