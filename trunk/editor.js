...
var helptexts = {
    ...
    "link_bandwidth_out":
    "The bandwidth from the second node to the first node (if that is different)",
...
function help_handler(e)
{
    ...
    if ((e.type == 'blur') || (e.type == 'mouseout')) {
        ...
        if (helptext == 'undefined') {
            alert('OID is: ' + objectid + ' and target is:' + target + ' and section is: '
                + section);
        }
    }
...
function do_submit() { document.frmMain.submit(); }

function cactipicker()
{
    // make sure it isn't already opened
    if (!newWindow || newWindow.closed) {
        newWindow = window.open("", "cactipicker",
            "scrollbars=1,status=1,height=400,width=400,resizable=1");
...
function new_file() { self.location = "?action=newfile"; }
...
function add_node()
{
    document.getElementById(
        'tb_help').innerText = 'Click on the map where you would like to add a new node.';
...
function move_node()
{
    ...
    document.getElementById('tb_help').innerText
        = 'Click on the map where you would like to move the node to.';
...
function via_link()
{
    ...
    document.getElementById('tb_help').innerText
        = 'Click on the map via which point you whant to redirect link.';
...
{
    document.getElementById(
        'tb_help').innerText = 'Click on the first node for one end of the link.';
...
{
    document.getElementById('tb_help').innerText
        = 'Click on the map where you would like to put the timestamp.';
    document.getElementById('action').value = "place_stamp";
    mapmode('xy');
}

// called from clicking the toolbar
function position_first_legend() { real_position_legend('DEFAULT'); }
...
{
    document.getElementById(
        'tb_help').innerText = 'Click on the map where you would like to put the legend.';
    document.getElementById('action').value = "place_legend";
...
function show_link(name)
{
    ...
    if (mylink) {
        ...
        if (jQuery('#link_commentposout').val() != mylink.commentposout) {
            jQuery('#link_commentposout').prepend("<option selected value='"
                + mylink.commentposout + "'>" + mylink.commentposout + "%</option>");
        }

        if (jQuery('#link_commentposin').val() != mylink.commentposin) {
            jQuery('#link_commentposin').prepend("<option selected value='"
                + mylink.commentposin + "'>" + mylink.commentposin + "%</option>");
        }
...
function show_dialog(dlg) { document.getElementById(dlg).style.display = 'block'; }
...
function coord_release(event) { jQuery('#tb_coords').html('Position<br />---, ---'); }
