...
if (isset($_REQUEST['mapname'])) {
    ...
    $mapname = wmeSanitizeConfigFile($mapname);
}
if (isset($_REQUEST['selected'])) {
    $selected = wmeSanitizeSelected($_REQUEST['selected']);
}
...
if ($mapname == '') {
    ...
    switch($action) {
        ...
        case 'newmapcopy':
            ...
            $sourcemapname = wmeSanitizeConfigFile($sourcemapname);
        ...
        case "set_node_properties":
            ...
            $map->nodes[$new_node_name]->label = wmeSanitizeString($_REQUEST['node_label']);
            $map->nodes[$new_node_name]->infourl[IN] = wmeSanitizeString($_REQUEST['node_infourl']);
        ...
        case "set_link_properties":
            ...
            if (strpos($link_name, " ") === false) {
                ...
                $map->links[$link_name]->infourl[IN] = wmeSanitizeString($_REQUEST['link_infourl']);
                $map->links[$link_name]->infourl[OUT] = wmeSanitizeString($_REQUEST['link_infourl']);
                ...
                $map->links[$link_name]->comments[IN] =  wmeSanitizeString($_REQUEST['link_commentin']);
                $map->links[$link_name]->comments[OUT] = wmeSanitizeString($_REQUEST['link_commentout']);
                ...
                if (wmeValidateBandwidth($bwin)) {
                    $map->links[$link_name]->max_bandwidth_in_cfg = $bwin;
                    $map->links[$link_name]->max_bandwidth_in = wmInterpretNumberWithMetricPrefix($bwin, $map->kilo);
                }
                if (wmeValidateBandwidth($bwout)) {
                    $map->links[$link_name]->max_bandwidth_out_cfg = $bwout;
                    $map->links[$link_name]->max_bandwidth_out = wmInterpretNumberWithMetricPrefix($bwout, $map->kilo);
                }
                $map->writeConfig($mapfile);
            }
            break;
        case "set_map_properties":
            ...
            $map->title = wmeSanitizeString($_REQUEST['map_title']);
            $map->keytext['DEFAULT'] = wmeSanitizeString($_REQUEST['map_legend']);
            $map->stamptext = wmeSanitizeString($_REQUEST['map_stamp']);

            $map->htmloutputfile = wmeSanitizeFile($_REQUEST['map_htmlfile'], array("html"));
            $map->imageoutputfile = wmeSanitizeFile($_REQUEST['map_pngfile'], array("png","jpg","gif","jpeg"));
            ...
            if ($_REQUEST['map_bgfile'] == '--NONE--') {
                ...
                $map->background = wmeSanitizeFile(stripslashes($_REQUEST['map_bgfile']), array("png","jpg","gif","jpeg"));
            }
            ...
            wmeHandleInheritance($map, $inheritables);
            ...
            if (($bwin_old != $bwin) || ($bwout_old != $bwout)) {
                ...
                $map->links['DEFAULT']->max_bandwidth_in = wmInterpretNumberWithMetricPrefix($bwin, $map->kilo);
                            $map->links['DEFAULT']->max_bandwidth_out = wmInterpretNumberWithMetricPrefix($bwout, $map->kilo);
        ...
        case 'set_map_style':
            ...
            if (wmeValidateOneOf($_REQUEST['mapstyle_htmlstyle'], array('static','overlib'), false)) {
        ...
        case "add_link2":
            ...
            break;
        case "place_legend":
            $x = wmeGridSnap(intval($_REQUEST['x']), $grid_snap_value);
            $y = wmeGridSnap(intval($_REQUEST['y']), $grid_snap_value);
            $scalename = wmeSanitizeName($_REQUEST['param']);
        ...
        case "via_link":
            $x = intval($_REQUEST['x']);
            $y = intval($_REQUEST['y']);
            $link_name = wmeSanitizeName($_REQUEST['link_name']);
        ...
        case "move_node":
            $x = wmeGridSnap(intval($_REQUEST['x']), $grid_snap_value);
            $y = wmeGridSnap(intval($_REQUEST['y']), $grid_snap_value);
            $node_name = wmeSanitizeName($_REQUEST['node_name']);
        ...
        case "link_align_horizontal":
            ...
            $target = wmeSanitizeName($_REQUEST['param']);
    ...
    if ($selected != '') {
        $imageurl .= '&amp;selected='.urlencode(wmeSanitizeSelected($selected));
    }
    ...
    <div id="dlgMapStyle" class="dlgProperties">
      ...
      <div class="dlgBody">
        <table>
          ...
          <tr>
            <th>Node Font</th>
            <td><?php echo wmeGetFontList($map, 'mapstyle_nodefont', $map->nodes['DEFAULT']->labelfont); ?></td>
          </tr>
          <tr>
            <th>Link Label Font</th>
            <td><?php echo wmeGetFontList($map, 'mapstyle_linkfont', $map->links['DEFAULT']->bwfont); ?></td>
          </tr>
          <tr>
            <th>Legend Font</th>
            <td><?php echo wmeGetFontList($map, 'mapstyle_legendfont', $map->keyfont); ?></td>
          </tr>
          <tr>
            <th>Font Samples:</th>
