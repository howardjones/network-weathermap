...
$cacti_graph
    = "$cacti_base/graph_image.php?local_graph_id=%d&rra_id=0&graph_nolegend=true&graph_height=100&graph_width=300";
...
while (<INPUT>) {
    if (m/^\s*LINK\s+(\S+)/i) {
        if ($overlibcount == 0 && $target ne "") { find_graph_urls($target); }

        $overlibcount = 0;
        $target = "";
    }

    if (m/^\s*TARGET\s+(\S+\.rrd)/i) { $target = $1; }

    if (m/^\s*OVERLIBGRAPH\s+(\S+)/i) { $overlibcount++; }
...
sub find_graph_urls {
    ...
    $SQL = "select local_data_id from data_template_data where data_source_path like '%"
        . $target . "' LIMIT 1";
    $sth = $dbh->prepare($SQL);
    $sth->execute();
    ...
    if ($count == 1) {
        ...
        print OUTPUT "\t#Automatically made. Graph is ID $local_graph_id: $title\n";
