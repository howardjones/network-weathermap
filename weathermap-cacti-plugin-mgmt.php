...
switch ($action) {
    ...
    case 'chgroup':
        if (isset($_REQUEST['id']) && is_numeric($_REQUEST['id'])) {
            require_once($config["base_path"]."/include/top_header.php");
            wmuiMapGroupChangePage(intval($_REQUEST['id']));
            require_once($config["base_path"]."/include/bottom_footer.php");
        } else {
    ...
    case 'map_settings_form':
        if (isset($_REQUEST['mapid']) && is_numeric($_REQUEST['mapid'])) {
            ...
            if (isset($_REQUEST['id']) && is_numeric($_REQUEST['id'])) {
                wmuiMapSettingsForm(intval($_REQUEST['mapid']), intval($_REQUEST['id']));
            } else {
                wmuiMapSettingsForm(intval($_REQUEST['mapid']));
            }
    ...
    case 'map_settings':
        if (isset($_REQUEST['id']) && is_numeric($_REQUEST['id'])) {
            require_once($config["base_path"]."/include/top_header.php");
            wmuiMapSettingsPage(intval($_REQUEST['id']));
            wmGenerateFooterLinks();
            require_once($config["base_path"]."/include/bottom_footer.php");
        }
        break;
    case 'perms_add_user':
        if (isset($_REQUEST['mapid']) && is_numeric($_REQUEST['mapid'])
            ...
            wmPermissionsUserAdd(intval($_REQUEST['mapid']), intval($_REQUEST['userid']));
    ...
    case 'perms_delete_user':
        if (isset($_REQUEST['mapid']) && is_numeric($_REQUEST['mapid'])
            ...
            wmPermissionsUserDelete($_REQUEST['mapid'], $_REQUEST['userid']);
    ...
    case 'perms_edit':
        if (isset($_REQUEST['id']) && is_numeric($_REQUEST['id'])) {
            require_once($config["base_path"]."/include/top_header.php");
            wmuiMapPermissionsPage($_REQUEST['id']);
            require_once($config["base_path"]."/include/bottom_footer.php");
        } else {
            print "Something got lost back there.";
        }
    ...
    case 'delete_map':
        if (isset($_REQUEST['id']) && is_numeric($_REQUEST['id'])) {
            wmMapDelete($_REQUEST['id']);
        }
        header("Location: weathermap-cacti-plugin-mgmt.php");
        break;
    case 'deactivate_map':
        if (isset($_REQUEST['id']) && is_numeric($_REQUEST['id'])) {
            wmMapDeactivate($_REQUEST['id']);
        }
        header("Location: weathermap-cacti-plugin-mgmt.php");
        break;
    case 'activate_map':
        if (isset($_REQUEST['id']) && is_numeric($_REQUEST['id'])) {
            wmMapActivate($_REQUEST['id']);
        }
        header("Location: weathermap-cacti-plugin-mgmt.php");
        break;
    case 'move_map_up':
        if (isset($_REQUEST['id']) && is_numeric($_REQUEST['id']) &&
            ...
            wmMapMove($_REQUEST['id'], $_REQUEST['order'], -1);
        }
        header("Location: weathermap-cacti-plugin-mgmt.php");
        break;
    case 'move_map_down':
    ...
    case 'move_group_up':
        if (isset($_REQUEST['id']) && is_numeric($_REQUEST['id']) &&
            isset($_REQUEST['order']) && is_numeric($_REQUEST['order'])) {
            wmMapGroupMove(intval($_REQUEST['id']), intval($_REQUEST['order']), -1);
        }
        header("Location: weathermap-cacti-plugin-mgmt.php?action=groupadmin");
        break;
    ...
    case 'viewconfig':
        ...
        if (isset($_REQUEST['file'])) {
            wmuiPreviewConfig($_REQUEST['file']);
        } else {
            print "No such file.";
        }
        require_once $config["base_path"]."/include/bottom_footer.php";
