...
include_once(dirname(__FILE__) . "/lib/all.php");
...
switch ($action) {
    ...
    case 'enable_poller_output':
        ...
        header("Location: weathermap-cacti88-plugin-mgmt.php?action=map_settings&id=0");
        break;
    ...
    case 'group_update':
        ...
        header("Location: weathermap-cacti88-plugin-mgmt.php?action=groupadmin");
    ...
    case 'map_settings_delete':
        ...
        header("Location: weathermap-cacti88-plugin-mgmt.php?action=map_settings&id=" . $mapid);
        break;
...
// this is the save option from the map_settings_form
    ...
    case 'perms_add_user':
        if (isset($_REQUEST['mapid']) && is_numeric($_REQUEST['mapid'])
            ...
            header("Location: weathermap-cacti88-plugin-mgmt.php?action=perms_edit&id=" . intval($_REQUEST['mapid']));
        }
        break;
    case 'perms_delete_user':
        if (isset($_REQUEST['mapid']) && is_numeric($_REQUEST['mapid'])
            ...
            header("Location: weathermap-cacti88-plugin-mgmt.php?action=perms_edit&id=" . $_REQUEST['mapid']);
        }
        break;
    case 'perms_edit':
        if (isset($_REQUEST['id']) && is_numeric($_REQUEST['id'])) {
    ...
    case 'editor':
        ...
        // include_once('./weathermap-cacti88-plugin-editor.php');
        break;
    ...
    case 'rebuildnow':
        ...
        print "<h4><a href=\"weathermap-cacti88-plugin-mgmt.php?action=rebuildnow2\">YES</a></h4>";
...
function weathermap_footer_links()
{
    ...
    print '<br />';
    html_start_box("<center><a target=\"_blank\" class=\"linkOverDark\" href=\"docs/\">Local Documentation</a> -- <a target=\"_blank\" class=\"linkOverDark\" href=\"http://www.network-weathermap.com/\">Weathermap Website</a> -- <a target=\"_target\" class=\"linkOverDark\" href=\"weathermap-cacti88-plugin-editor.php\">Weathermap Editor</a> -- This is version " . WEATHERMAP_VERSION . "</center>",
...
function maplist()
{
    ...
    html_start_box("<strong>Weathermaps</strong>", "78%", $colors["header"], "3", "center",
        "weathermap-cacti88-plugin-mgmt.php?action=addmap_picker");
...
//			$userlist = db_fetch_assoc($UserSQL);
    ...
    print "<a href='weathermap-cacti88-plugin-mgmt.php?action=groupadmin'><img src='images/button_editgroups.png' border=0 alt='Edit Groups' /></a>";
...
function weathermap_map_settings($id)
{
    ...
    html_start_box("<strong>$title</strong>", "70%", $colors["header"], "2", "center",
        "weathermap-cacti88-plugin-mgmt.php?action=map_settings_form&mapid=" . intval($id));
...
function weathermap_map_settings_form($mapid = 0, $settingid = 0)
{
    ...
    form_save_button("weathermap-cacti88-plugin-mgmt.php?action=map_settings&id=" . $mapid);
...
function weathermap_chgroup($id)
{
    ...
    html_start_box("<strong>Edit map group for Weathermap $id: $title</strong>", "70%", $colors["header"], "2",
        "center", "");
...
function weathermap_group_editor()
{
    ...
    html_start_box("<strong>Edit Map Groups</strong>", "70%", $colors["header"], "2", "center",
        "weathermap-cacti88-plugin-mgmt.php?action=group_form&id=0");
    ...
    if (is_array($groups)) {
        if (sizeof($groups) > 0) {
            foreach ($groups as $group) {
                form_alternate_row_color($colors["alternate"], $colors["light"], $n);
                print '<td><a href="weathermap-cacti88-plugin-mgmt.php?action=group_form&id=' . intval($group['id']) . '"><img src="../../images/graph_properties.gif" width="16" height="16" border="0" alt="Rename This Group" title="Rename This Group">Rename</a></td>';
                ...
                print '<a href="weathermap-cacti88-plugin-mgmt.php?action=move_group_up&order=' . $group['sortorder'] . '&id=' . $group['id'] . '"><img src="../../images/move_up.gif" width="14" height="10" border="0" alt="Move Group Up" title="Move Group Up"></a>';
                print '<a href="weathermap-cacti88-plugin-mgmt.php?action=move_group_down&order=' . $group['sortorder'] . '&id=' . $group['id'] . '"><img src="../../images/move_down.gif" width="14" height="10" border="0" alt="Move Group Down" title="Move Group Down"></a>';
// print $map['sortorder'];
                ...
                print '<td>';
                if ($group['id'] > 1) {
                    print '<a href="weathermap-cacti88-plugin-mgmt.php?action=groupadmin_delete&id=' . intval($group['id']) . '"><img src="../../images/delete_icon.gif" width="10" height="10" border="0" alt="Remove this definition from this map"></a>';
                }
                print '</td>';
