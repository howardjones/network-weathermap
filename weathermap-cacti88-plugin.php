...
switch ($action) {
    ...
    case 'mrss':
        ...
        foreach ($maplist as $map) {
            $thumburl = "weathermap-cacti88-plugin.php?action=viewthumb&id=" . $map['filehash'] . "&time=" . time();
            $bigurl = "weathermap-cacti88-plugin.php?action=viewimage&id=" . $map['filehash'] . "&time=" . time();
            $linkurl = 'weathermap-cacti88-plugin.php?action=viewmap&id=' . $map['filehash'];
    ...
    if (sizeof($map)) {
        ...
        if (file_exists($htmlfile)) {
            ...
            if (isset($user_auth_realm_filenames[basename('weathermap-cacti88-plugin.php')])) {
                $realm_id2 = $user_auth_realm_filenames[basename('weathermap-cacti88-plugin.php')];
            }
...
function weathermap_show_manage_tab()
{
    ...
    if (isset($user_auth_realm_filenames['weathermap-cacti88-plugin-mgmt.php'])) {
        $realm_id2 = $user_auth_realm_filenames['weathermap-cacti88-plugin-mgmt.php'];
    }
    $userid = (isset($_SESSION["sess_user_id"]) ? intval($_SESSION["sess_user_id"]) : 1);
    ...
    <div id="wmcyclecontrolbox" class="<?php print $class ?>">
        ...
        <div id="wm_cyclecontrols">
            ...
            <a id="cycle_next" href="#"><img src="cacti-resources/img/control_fastforward_blue.png" width="16"
                                             height="16"/></a>
            <a id="cycle_fullscreen" href="?action=viewmapcycle&fullscreen=1&group=<?php echo $limit_to_group; ?>"><img
...
function weathermap_footer_links()
{
    global $colors;
    print '<br />';
    html_start_box("<center><a target=\"_blank\" class=\"linkOverDark\" href=\"docs/\">Local Documentation</a> -- <a target=\"_blank\" class=\"linkOverDark\" href=\"http://www.network-weathermap.com/\">Weathermap Website</a> -- <a target=\"_target\" class=\"linkOverDark\" href=\"weathermap-cacti88-plugin-editor.php?plug=1\">Weathermap Editor</a> -- This is version " . WEATHERMAP_VERSION . "</center>",
